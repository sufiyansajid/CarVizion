...
import { cn } from "@/lib/utils"; // Changed from "../lib/utils" to "@/lib/utils" for consistency
...
export const HamburgerMenuOverlay: React.FC<HamburgerMenuOverlayProps> = ({
  items = [],
  buttonTop = "60px",
  buttonLeft, // Removed default value to allow 'undefined' for right alignment check
  buttonSize = "md",
  buttonColor = "#6c8cff",
  ...
  const buttonSizes = {
    sm: "w-11 h-11", // 44px for touch targets
    md: "w-12 h-12",
    lg: "w-16 h-16",
  };
  ...
  const buttonStyle = {
    top: buttonTop,
    ...(buttonLeft ? { left: buttonLeft } : { right: "20px" }), // Default to right if not specified
    position: "absolute" as const,
    zIndex: zIndex + 1,
  };

  // Convert position for clip-path center
  // Note: clip-path requires specific coordinates. If using calc/right, this might be tricky in pure CSS string.
  // We can try to rely on CSS variables or just use the passed props if they are valid css values.
  // For 'right' positioning, we might need a fixed left value or use 'at right top' etc.
  
  // Simplification: Let's assume the button is always at the passed position.
  // If buttonLeft is passed, use it. If not, we might need to change the clip-path origin.
  
  const clipPathOrigin = buttonLeft 
    ? `${buttonLeft} ${buttonTop}` 
    : `calc(100% - 20px) ${buttonTop}`;
  ...
  return (
    <div ref={containerRef} className={cn("absolute inset-0 pointer-events-none", className)}>
      <style>
        {`
          ...
          .hamburger-overlay-${zIndex} {
            ...
            z-index: ${zIndex};
            clip-path: circle(0px at ${clipPathOrigin});
          ...
          .hamburger-overlay-${zIndex}.open {
            clip-path: circle(150% at ${clipPathOrigin});
          }
          ...
          @media (max-width: 768px) {
            .hamburger-button-${zIndex} {
              /* Ensure these match props if needed, or rely on JS props */
            }
            
            .hamburger-overlay-${zIndex} {
              /* clip-path populated by JS props */
            }
          }
        `}
      </style>
      ...
      <button
        ...
        style={buttonStyle} // Apply dynamic positioning style
      >
        {customButton || (
